...
function [ zRed, zGreen, zBlue, sampleIndices ] = makeImageMatrix( filenames, numPixels )
  ...
  % zLuminance = zeros(numSamples, numExposures);
    ...
    for i=1:numExposures
        ...
        fprintf('Reading image number %i...%s ', i,imagePath);

        image = im2uint8(imread(imagePath));
        fprintf('sampling.\n');
        ...
        zGreen(:,i) = zGreenTemp;
        zBlue(:,i) = zBlueTemp;       
        %zLuminance(:,i)= 0.299 * zRedTemp + 0.587 * zGreenTemp + 0.114 * zBlueTemp;

    end
    %zLuminance=uint8(zLuminance);
